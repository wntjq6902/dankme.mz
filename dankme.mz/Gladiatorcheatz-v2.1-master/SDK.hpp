...
#include "misc/cl_junkcode.hpp"

#include "NetMngr.hpp"

#include "helpers/Config.hpp"
#include "helpers/SanityChech.hpp"
...
namespace Global
{
	...
	extern int iHwidIndex;
	extern bool bIsPanorama;
	...
	extern float lastdmg[65];

	extern float realyaw, fakeyaw, realpitch, fakepitch, lasthc, lastawdmg;

	extern bool hit_while_brute[65];

	extern int aimbot_target;
	extern int aim_hitbox;

	extern double LastLBYUpdate, NextLBYUpdate;
	...
	extern bool use_ud_vmt;

	extern int fps;
	extern Vector vecAimpos;

	extern int PlayersChockedPackets[65];
	extern Vector FakelagUnfixedPos[65];
	extern bool FakelagFixed[65];

	extern bool bShouldUnload;

	extern std::string currentLUA;
}
...
namespace Proxies
{
	...
	void storePlayerPitch(const CRecvProxyData *pData, void *pStruct, void *pOut);
	void storePlayeryaw(const CRecvProxyData *pData, void *pStruct, void *pOut);
	void LBYUpdated(const CRecvProxyData *pData, void *pStruct, void *pOut);
}
