...
enum CSWeaponType
{
	WEAPONTYPE_KNIFE = 1,	//weapon type enum pushed by +1 on panorama
	WEAPONTYPE_PISTOL,
	WEAPONTYPE_SUBMACHINEGUN,
	WEAPONTYPE_RIFLE,
//	WEAPONTYPE_SHOTGUN,	these two got removed on panorama
...
{
public:
	/*	outdated after panorama update.
	char _0x0000[20];
	int32 m_iClip1;            //0x0014 
	char _0x0018[12];
	int32 max_reserved_ammo;    //0x0024 
	char _0x0028[96];
	char* hud_name;                //0x0088 
	char* weapon_name;            //0x008C 
	char _0x0090[60];
	int32 weapon_type;                //0x00CC 
	int32 price;                //0x00D0 
	int32 reward;                //0x00D4 
	char _0x00D8[20];
	BYTE full_auto;                //0x00EC 
	char _0x00ED[3];
	int32 m_iDamage;                //0x00F0 
	float m_fArmorRatio;            //0x00F4 
	int32 bullets;            //0x00F8 
	...
	char _0x0138[76];
	int32 recoil_seed;        //0x0184 
	char _0x0188[32];

	virtual ~WeapInfo_t() {};	also wrong, thanks parkju
	char* weapon_name; //0x0004 
	char pad_0x0008[0x8]; //0x0008
	char pad_0x0010[0x2];//CCStrike15ItemDefinition* m_pItemDefinition; //0x0010 we don't have definition for this and we're using netvar for item definition anyway
	int m_iClip1; //0x0014 
...
{
public:
	NETVAR(int32_t, m_nBombSite, "CPlantedC4", "m_nBombSite");
	NETVAR(float_t, m_flC4Blow, "CPlantedC4", "m_flC4Blow");
	NETVAR(float_t, m_flDefuseLength, "CPlantedC4", "m_flDefuseLength");
	NETVAR(float_t, m_flDefuseCountDown, "CPlantedC4", "m_flDefuseCountDown");
...
public:
	std::uint8_t* load_image(unsigned short id, int width, int height, const std::string &type = "PNG")
	{
		...
		auto resource = FindResource(Global::hmDll, MAKEINTRESOURCE(id), type.data());
		auto loaded_resource = LoadResource(Global::hmDll, resource);
		auto resource_ptr = LockResource(loaded_resource);
		auto resource_size = SizeofResource(Global::hmDll, resource);
	...
	C_Texture(unsigned short id, int width, int height, const std::string &type = "PNG")
		...
		, height_(height)
	{ }
	C_Texture(uint8_t *data_in, int width, int height)
		: data_(data_in)
		, width_(width)
		, height_(height)
	{ }
	...
	static void paint_raw(int x, int y, int w, int h, uint8_t *data)
	{
		static unsigned int texture_id;
		if (!g_VGuiSurface->IsTextureIDValid(texture_id))
		{
			texture_id = g_VGuiSurface->CreateNewTextureID(true);
		}
		g_VGuiSurface->DrawSetColor(255, 255, 255, 255);
		g_VGuiSurface->DrawSetTextureRGBA(texture_id, data, uint32_t(w), uint32_t(h));
		g_VGuiSurface->DrawTexturedRect(x, y, x + w, y + h);
	}
};
...
public:
	...
	NETVAR(int16_t, m_iItemDefinitionIndex, "CBaseAttributableItem", "m_iItemDefinitionIndex");
...
public:
	...
	NETVAR(bool, m_bIsPlayerGhost, "CBasePlayer", "m_bIsPlayerGhost");
	...
	{
		static unsigned int offset = NetMngr::Get().getOffs("DT_BasePlayer", "m_hConstraintEntity") - 0xc;
	...
	bool SetupBonesExperimental(matrix3x4_t *pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime, bool fakelag_fix = false);
	bool RebuiltSetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime);
};
