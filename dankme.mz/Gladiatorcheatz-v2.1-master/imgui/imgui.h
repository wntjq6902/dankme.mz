...
namespace ImGui
{
	...
	IMGUI_API bool		    ColorButton(const char* desc_id, const float col[], ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0));
	...
	IMGUI_API bool          BeginPopup(const char* str_id, int Flags = 0);                      // return true if the popup is open, and you can start outputting to it. only call EndPopup() if BeginPopup() returned true!
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_OnlyDragByTitleBar     = 1 << 17,
    // [Internal]
...
struct ImGuiStyle
{
	...
	float		WindowPadThickness;			// If more than 0, will add an outline the likes of the title bar around the whole window. rounding capabilities included.
