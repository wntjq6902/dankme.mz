...
{
	if (!g_Options.legit_enabled && !g_Options.legit_rcs && !g_Options.legit_backtrack)
		return;
	...
	if (g_Options.legit_enabled && ((g_Options.legit_aimkey1 != BUTTON_CODE_NONE && g_Options.legit_aimkey2 != BUTTON_CODE_NONE) ? (g_InputSystem->IsButtonDown(static_cast<ButtonCode_t>(g_Options.legit_aimkey1)) || g_InputSystem->IsButtonDown(static_cast<ButtonCode_t>(g_Options.legit_aimkey2))) : true) && !g_LocalPlayer->m_hActiveWeapon().Get()->IsWeaponNonAim())
	{
		GetBestTarget();
		if (HasTarget())
		{
	...
	if (g_Options.legit_backtrack && usercmd->buttons & IN_ATTACK)
...
void AimLegit::Triggerbot()
{
	...
	junkcode::call();
	if ((curTime - timestamp) < g_Options.legit_trigger_delay && g_Options.legit_enabled && tb)
	{
		usercmd->buttons &= ~IN_ATTACK;
	}
}
