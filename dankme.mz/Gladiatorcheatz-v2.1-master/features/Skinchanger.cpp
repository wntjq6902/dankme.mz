...
const std::map<size_t, Item_t> k_weapon_info =
{
	...
	{ WEAPON_KNIFE_M9BAYONET,{ "models/weapons/v_knife_m9_bay.mdl", "knife_m9_bayonet" } },
	...
	{ WEAPON_KNIFE_DAGGER,{ "models/weapons/v_knife_push.mdl", "knife_push" } },
...
const std::vector<WeaponName_t> k_knife_names =
{
	...
	{ WEAPON_KNIFE_M9BAYONET, "M9 Bayonet" },
	{ WEAPON_KNIFE_HUNTSMAN, "Huntsman Knife" },
...
void Skinchanger::Work()
{
			...
			if (!glove)
			{
				...
				// He he
				{
					static auto set_abs_origin_fn = reinterpret_cast< void(__thiscall*)(void*, const Vector&) >
						(Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "55 8B EC 83 E4 F8 51 53 56 57 8B F1"));
	...
	// Handle weapon configs
	{
		...
		for (size_t i = 0; weapons[i].IsValid(); i++)
		{
			...
			if (auto active_conf = this->GetByDefinitionIndex(IsKnife(definition_index) ? WEAPON_KNIFE_CT : definition_index))
...
{
	std::string fPath = std::string(Global::my_documents_folder) + "\\DANKMEMZ_Repasted\\" + "skinchanger.json";
