	...
	for (auto i = 0; i < g_GlowObjManager->m_GlowObjectDefinitions.Count(); i++)
	{
		...
		case ClassId_CCSPlayer:
		{
			...
			if (entity == g_LocalPlayer)
				cur_color = vis ? g_Options.glow_player_color_local_visiable : g_Options.glow_player_color_local;
			else
				cur_color = (playerTeam ? (vis ? g_Options.glow_player_color_t_visible : g_Options.glow_player_color_t) : (vis ? g_Options.glow_player_color_ct_visible : g_Options.glow_player_color_ct));
