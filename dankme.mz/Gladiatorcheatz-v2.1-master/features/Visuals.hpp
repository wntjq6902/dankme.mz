...
namespace Visuals
{
	...
	void RenderLBYLC();
	void Radar();
	void DrawRecoilCrosshair(QAngle punch);
	void DrawSpreadCrosshair();
	void DrawAngleLines();
	void DrawWatermark();
	void DrawWeaponRange();
	...
	void DrawTriangle(int x, int y, float w, float h, float angle, Color color);
	void DrawSemiCircle(Vector2D center, Color color, float radius, float startang, float endang, float points);
