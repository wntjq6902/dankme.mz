...
void RebuildGameMovement::VelocityExtrapolate(C_BasePlayer *player, Vector &origin, Vector &velocity, int &flags, bool was_in_air)
{
	...
	float frametime_simulated = g_GlobalVars->interval_per_tick * host_timescale->GetFloat();

	if (!(flags & FL_ONGROUND))
		velocity.z -= (frametime_simulated * g_CVar->FindVar("sv_gravity")->GetFloat());
	else if (was_in_air)
		velocity.z = g_CVar->FindVar("sv_jump_impulse")->GetFloat();
	else if (!checks::is_bad_ptr(player->m_hActiveWeapon().Get()) && !checks::is_bad_ptr(player->m_hActiveWeapon().Get()->GetWeapInfo()))
	{
		if (velocity.Length2D() > player->m_hActiveWeapon().Get()->GetWeapInfo()->max_speed)
		{
			float speed = player->m_hActiveWeapon().Get()->GetWeapInfo()->max_speed;
			QAngle angle; Math::VectorAngles(velocity, angle);
			velocity = Vector(speed * cos(DEG2RAD(angle.yaw)), speed * sin(DEG2RAD(angle.yaw)), 0);
		}
	}

	Vector src = origin;
	Vector end = (src + velocity * frametime_simulated);
...
Vector RebuildGameMovement::VelocityExtrapolate(C_BasePlayer* pTarget, int amountOfTicks)
{
	Vector origin = pTarget->m_vecOrigin();
	Vector velocity = pTarget->m_vecVelocity();
	int flags = pTarget->m_fFlags();
	bool was_in_air = false;

	for (int i = 0; i < amountOfTicks; i++)
	{
		int last_flag = flags;
		Vector originbak = origin, velocitybak = velocity;
		VelocityExtrapolate(pTarget, origin, velocity, flags, was_in_air);
		if (abs((originbak - origin).Length()) > 64)	//we're making our own lc failure wtf
		{
			origin = originbak; velocity = velocitybak;
			break;
		}
