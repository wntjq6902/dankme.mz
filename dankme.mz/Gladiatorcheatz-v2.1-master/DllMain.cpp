...
BOOL __stdcall DllMain(HINSTANCE mod, DWORD dwReason, LPVOID res)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		junkcode::call();
		if (FileExists(std::string("dankme.mz_repasted.log")))
			std::remove("dankme.mz_repasted.log");

		Global::hmDll = mod;
		SetUnhandledExceptionFilter(ExceptionFilter);

		Global::handleDll = CreateThread(0, 0, &Installer::installGladiator, 0, 0, 0);
...
{
	Utils::Log("-= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -= -");
	Utils::Log("DankMe.Ms Repasted has crashed");
