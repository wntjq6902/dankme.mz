...
bool C_BaseEntity::IsPlayer()
{
	...
	return GetClientClass()->m_ClassID == ClassId::ClassId_CCSPlayer;
}
...
WeapInfo_t *C_BaseCombatWeapon::GetWeapInfo()
{
	...
	return VT::vfunc<o_getWeapInfo>(this, 448)(this); //https://www.unknowncheats.me/forum/counterstrike-global-offensive/280659-cs-beta-branch-update-changes-hitbox-struct.html
}
...
{
	static auto inReload = *(uint32_t*)(Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "C6 87 ? ? ? ? ? 8B 06 8B CE FF 90") + 2);
...
bool C_BaseCombatWeapon::IsMiscellaneousWeapon()
{
	...
	return
		(iWeaponID == WEAPON_KNIFE_CT || iWeaponID == WEAPON_C4
			|| iWeaponID == WEAPON_HE || iWeaponID == WEAPON_DECOY
			|| iWeaponID == WEAPON_FLASH || iWeaponID == WEAPON_MOLOTOV
			|| iWeaponID == WEAPON_KNIFE_FLIP || iWeaponID == WEAPON_KNIFE_GUT
			|| iWeaponID == WEAPON_KNIFE_T || iWeaponID == WEAPON_KNIFE_BAYONET
			|| iWeaponID == WEAPON_SMOKE || iWeaponID == WEAPON_INC
			|| iWeaponID == WEAPON_KNIFE_KARAMBIT || iWeaponID == WEAPON_KNIFE_M9BAYONET
...
bool C_BaseCombatWeapon::IsPistol()
{
	...
	*/
	int iWeaponID = m_iItemDefinitionIndex();
	return (iWeaponID == WEAPON_CZ75 || iWeaponID == WEAPON_DEAGLE ||
		iWeaponID == WEAPON_DUALS || iWeaponID == WEAPON_FIVE7 ||
		iWeaponID == WEAPON_GLOCK || iWeaponID == WEAPON_P2000 ||
		iWeaponID == WEAPON_P250 || iWeaponID == WEAPON_REVOLVER ||
		iWeaponID == WEAPON_TEC9 || iWeaponID == WEAPON_USPS);
}
...
bool C_BaseCombatWeapon::IsSniper()
{
	...
	*/
	int iWeaponID = m_iItemDefinitionIndex();
	return(iWeaponID == WEAPON_AWP || iWeaponID == WEAPON_SCOUT ||
		iWeaponID == WEAPON_SCAR20 || iWeaponID == WEAPON_G3SG1);
}
...
bool C_BaseCombatWeapon::IsGrenade()
{
	...
	*/
	int iWeaponID = m_iItemDefinitionIndex();
	return(iWeaponID == WEAPON_HE || iWeaponID == WEAPON_INC ||
		iWeaponID == WEAPON_MOLOTOV || iWeaponID == WEAPON_DECOY ||
		iWeaponID == WEAPON_FLASH || iWeaponID == WEAPON_SMOKE);
}

float C_BaseCombatWeapon::GetInaccuracy()
{
	//return VT::vfunc<float(__thiscall*)(void*)>(this, 470)(this);
	return VT::vfunc<float(__thiscall*)(void*)>(this, 471)(this); //https://www.unknowncheats.me/forum/counterstrike-global-offensive/280659-cs-beta-branch-update-changes-hitbox-struct.html
}
...
bool C_BaseCombatWeapon::IsWeaponNonAim()
{
	...
	return (idx == WEAPON_C4 || idx == WEAPON_KNIFE_CT || idx == WEAPON_KNIFE_BAYONET || idx == WEAPON_KNIFE_BUTTERFLY || idx == WEAPON_KNIFE_FALCHION
		|| idx == WEAPON_KNIFE_FLIP || idx == WEAPON_KNIFE_GUT || idx == WEAPON_KNIFE_KARAMBIT || idx == WEAPON_KNIFE_M9BAYONET || idx == WEAPON_KNIFE_DAGGER
		|| idx == WEAPON_KNIFE_BOWIE || idx == WEAPON_KNIFE_T || idx == WEAPON_KNIFE_HUNTSMAN || idx == WEAPON_FLASH || idx == WEAPON_HE
		|| idx == WEAPON_SMOKE || idx == WEAPON_MOLOTOV || idx == WEAPON_DECOY || idx == WEAPON_INC);
}
...
{
	return (C_CSGOPlayerAnimState*)((uintptr_t)this + 0x3894);
}
...
void C_BasePlayer::UpdateAnimationState(C_CSGOPlayerAnimState *state, QAngle angle)
{
	...
	static auto UpdateAnimState = (fnUpdateAnimState)Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24");
	if (!UpdateAnimState)
		return;

	UpdateAnimState(state, NULL, angle.roll, angle.yaw, angle.pitch, NULL);
	/*
	__asm
	{
		mov ecx, state
...
void C_BasePlayer::ResetAnimationState(C_CSGOPlayerAnimState *state)
{
	...
	static auto ResetAnimState = (ResetAnimState_t)Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "56 6A 01 68 ? ? ? ? 8B F1");
	if (!ResetAnimState)
		return;
...
void C_BasePlayer::CreateAnimationState(C_CSGOPlayerAnimState *state)
{
	...
	static auto CreateAnimState = (CreateAnimState_t)Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "55 8B EC 56 8B F1 B9 ? ? ? ? C7 46");
	if (!CreateAnimState)
		return;
...
Vector C_BasePlayer::GetEyePos()
{
	...
	{
		//return m_vecOrigin() + m_vecViewOffset(); outdated, inaccurate
		Vector vec;
		VT::vfunc<void(__thiscall*)(void*, Vector&)>(this, 279)(this, vec);
		return vec;
	}
}
...
bool C_BasePlayer::HasC4()
{
	static auto fnHasC4
		= reinterpret_cast<bool(__thiscall*)(void*)>(
			Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "56 8B F1 85 F6 74 31")
			);
...
Vector C_BasePlayer::GetBonePos(int bone)
{
	...
	if (SetupBonesExperimental(boneMatrix, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, g_EngineClient->GetLastTimeStamp())) {
...
int C_BasePlayer::GetBoneByName(const char *boneName)
{
	...
	if (!this->SetupBonesExperimental(boneToWorldOut, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, g_EngineClient->GetLastTimeStamp()))
		return -1;
...
Vector C_BasePlayer::GetHitboxPos(int hitbox)
{
	...
	if (this->SetupBonesExperimental(boneMatrix, 128, BONE_USED_BY_HITBOX, g_EngineClient->GetLastTimeStamp()))
	{
		studiohdr_t *studioHdr = g_MdlInfo->GetStudiomodel(this->GetModel());
		if (studioHdr)
		{
...
void C_BasePlayer::SetAbsOrigin(const Vector &origin)
{
	...
	static SetAbsOriginFn SetAbsOrigin = (SetAbsOriginFn)Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8");
...
void C_BasePlayer::SetAbsAngles(const QAngle &angles)
{
	...
	static SetAbsAnglesFn SetAbsAngles = (SetAbsAnglesFn)Utils::PatternScan(GetModuleHandle((Global::bIsPanorama ? "client_panorama.dll" : "client.dll")), "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8");
